...
// CUDA Kernel for matrix-vector multiplication with bias (2D indexing)
...
// CUDA Kernel for Tanh activation (2D indexing)
__global__ void Tanh_cuda(float* data, const float* bias, int size) {
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row < size) {
        data[row] = tanhf(data[row] + bias[row]);
    }
}
...
// CUDA Kernel for Softmax (2D indexing)
__global__ void softmax_1_cuda(float* output, float* softmax_out, int output_size) {
    ...
    if (row < output_size) {
        ...
        softmax_out[row] = expf(output[row] - max_val) / sum_exp;
    }
}
