...
__global__ void fused_relu_0_cuda(const float* __restrict__ mat, const float* __restrict__ vec, const float* __restrict__ bias, float* __restrict__ output, int rows, int cols) {
...
__global__ void addmm_1_cuda(const float* __restrict__ mat, const float* __restrict__ vec, const float* __restrict__ bias, float* __restrict__ output, int rows, int cols) {
    extern __shared__ float shared_vec[];
    int row = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void softmax_2_cuda(float* __restrict__ output, float* __restrict__ softmax_out, int output_size) {
    int idx = threadIdx.x;
...
int main() {
    ...
    // Launch fused_relu_0_cuda kernel with grid size (313,1,1) and block size (128,1,1)
    dim3 fused_relu_gridDim(313, 1, 1);
    dim3 fused_relu_blockDim(128, 1, 1);
    fused_relu_0_cuda<<<fused_relu_gridDim, fused_relu_blockDim, input_size * sizeof(float)>>>(d_linear1_weights, d_input, d_linear1_bias, d_hidden, hidden_size, input_size);

    // Launch addmm_1_cuda kernel with grid size (4,1,1) and block size (64,1,1)
    dim3 addmm_gridDim(4, 1, 1);
    dim3 addmm_blockDim(64, 1, 1);
    addmm_1_cuda<<<addmm_gridDim, addmm_blockDim, input_size * sizeof(float)>>>(d_linear2_weights, d_hidden, d_linear2_bias, d_output, output_size, hidden_size);

    // Launch softmax_2_cuda kernel with grid size (1,1,1) and block size (64,1,1)
