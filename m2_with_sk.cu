...
__global__ void mm_cuda(const float* mat, const float* vec, float* output, int rows, int cols) {
    // Thread index computation based on grid and block size
...
__global__ void addmm_cuda(const float* mat, const float* vec, const float* bias, float* output, int rows, int cols) {
    // Row index computation using grid and block dimensions
...
int main() {
    ...
    // Launch mm_cuda kernel with grid size (5000,1,1) and block size (32,4,1)
    ...
    // Launch ReLU_cuda kernel with grid size (79,1,1) and block size (128,1,1)
    ...
    // Launch addmm_cuda kernel with grid size (25,1,4) and block size (34,4,1)
    ...
    // Launch softmax_1_cuda kernel with grid size (1,1,1) and block size (64,1,1)
