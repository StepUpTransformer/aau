...
int main() {
    ...
    // Kernel mm_cuda: Grid size (5000, 1, 1), Block size (32, 4, 1)
    dim3 mm_gridDim(5000, 1, 1);  // Grid size as specified
    dim3 mm_blockDim(32, 4, 1);   // Block size as specified
    mm_cuda<<<mm_gridDim, mm_blockDim>>>(d_linear1_weights, d_input, d_hidden, hidden_size, input_size);

    // Kernel ReLU_cuda: Grid size (79, 1, 1), Block size (128, 1, 1)
    dim3 ReLU_gridDim(79, 1, 1);  // Grid size as specified
    dim3 ReLU_blockDim(128, 1, 1); // Block size as specified
    ReLU_cuda<<<ReLU_gridDim, ReLU_blockDim>>>(d_hidden, d_linear1_bias, hidden_size);

    // Kernel addmm_cuda: Grid size (25, 1, 4), Block size (34, 4, 1)
    dim3 addmm_gridDim(25, 1, 4);  // Grid size as specified
    dim3 addmm_blockDim(34, 4, 1); // Block size as specified
    addmm_cuda<<<addmm_gridDim, addmm_blockDim>>>(d_linear2_weights, d_hidden, d_linear2_bias, d_output, output_size, hidden_size);

    // Kernel softmax_1_cuda: Grid size (1, 1, 1), Block size (64, 1, 1)
    dim3 softmax_gridDim(1, 1, 1);  // Grid size as specified
    dim3 softmax_blockDim(64, 1, 1); // Block size as specified
